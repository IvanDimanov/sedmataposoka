...
function getUserByRequestID($request_id) {

  /*Access the common DB connection handler*/
  require_once('./models/db_manager.php');
  $db = getDBConnection();

  /*Try to find the user by its unique request ID*/
  $query  = $db->prepare('SELECT * FROM user WHERE request_id = :request_id LIMIT 1');
...
/*
  ...
  by a criteria of 'email' AND 'password' and have a 'is_active === true',
...
function authenticate($email, $password) {
  ...
  /*Try to find the user by its unique email*/
  $query  = $db->prepare('SELECT * FROM user WHERE email=:email AND password=:password AND is_active = 1 LIMIT 1');
  $result = $query->execute(array(
    'email'    => $email,
    'password' => $password
  ));
...
function createUser($new_user) {
  ...
  */
  ...
  if (!$result ||
      !getUserByRequestID( $new_user['request_id'] )
  ) {
...
function updateUser($request_id, $update_data) {
  ...
  if (!getUserByRequestID( $request_id )) {
