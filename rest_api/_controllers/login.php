...
/*Secure correct this file usage for Login policy only*/
if (sizeof( $request['url_conponents'] )  != 1 ||
            $request['url_conponents'][0] != 'login'
) {
  header('HTTP/1.1 400 Bad Request');
  die('{"error":"Login service for method \''.$request['method'].'\' and URL \''.$request['url'].'\' not found"}');
}
...
function clearLogin() {
  ...
  if (!isset(     $user_request['email']) ||
      !filter_var($user_request['email'], FILTER_VALIDATE_EMAIL)
  ) {
    header('HTTP/1.1 400 Bad Request');
    die('{"login":false}');
  }
